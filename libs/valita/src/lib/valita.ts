...
const fileSystem: v.Type<FileSystem> = v.lazy(() => v.union(
  v.object({
    type: v.literal('directory'),
    children: v.array(fileSystem).assert(a => new Set(a.map(f => f.name)).size === a.length, 'expected unique names in the children'),
...
const isoDate = v.string().assert(s => ISO_DATE_REGEX.test(s), `expected iso date of format ${ISO_DATE_REGEX_S}`).map(s => new Date(s));
