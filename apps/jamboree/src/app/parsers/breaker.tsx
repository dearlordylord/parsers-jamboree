...
import { BREAKERS, BREAKER_DESCRIPTIONS } from '@parsers-jamboree/checker/breaker';
...
export const Breaker = ({decodeUser}: Props): React.ReactElement => {
  ...
  return (
    <div>
      <table>
        <tr>{Object.entries(BREAKERS).map(([k]) => {
          return (

            <th title={BREAKER_DESCRIPTIONS[k as keyof typeof BREAKERS]} key={k}>{k}</th>

          );
        })}</tr>
          <tr>{Object.entries(BREAKERS).map(([k, f]) => {
          const parsed = decodeUser(f(igor));
          return (
            <td key={k}>{parsed._tag === 'left' ? <IconHeart color="red"/> : <IconHeartBroken/>}</td>
          );
          })}</tr>
      </table>
    </div>
